...
- To use the iOS app, open the Xcode project (SnapOSLocationToolsiOS/app/iOS/SpectaclesKitSample/SpectaclesKitSample.xcodeproj), build and deploy it to an iOS device, grant the location permission, then use the Mobile Kit bind functionality to pair the iOS app and the Spectacles Lens. Once bound, first 'Start Mobile Kit' in the Lens, then in the iOS app start a Mobile Kit session, then start sending subscription messages. This will send the latest location information from the iOS device to the Lens once per second.
